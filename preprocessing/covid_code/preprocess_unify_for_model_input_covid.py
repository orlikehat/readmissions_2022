...
IS_RUN_DIAGNOSES=True
IS_RUN_VS=True
IS_RUN_BLOOD_COUNT=True   
IS_RUN_BLOOD_CHEM=True
IS_RUN_BLOOD_COAG=True
IS_RUN_VENTILATION=True
IS_RUN_CCI=True
IS_RUN_MICROBIOLOGY=True
IS_RUN_SURGERY=True
IS_RUN_HOSP_PREV_YEAR=False#super slow so no option of running here
IS_RUN_ISHPUZIM=True
IS_RUN_DIAGNOSES_BG=True
IS_RUN_NORTON=True
IS_RUN_PROCEDURES=True
IS_RUN_MEDS_HOSP=False
IS_RUN_MEDS_ADM=False 
IS_RUN_HS=True
IS_RUN_DIAG_MANUAL=True
IS_RUN_DIAG_COUNT=True
IS_RUN_MEDS_COUNT=True
...
if IS_RUN_HOSP_PREV_YEAR:
    ...
    df_prev=pd.read_pickle(r"O:\OrlI\readmissions\preprocessed\adm_previous_year\adm_previous_year_covid.pkl")
...
if IS_RUN_ISHPUZIM:
    ...
    df_ishpuzim=pd.read_pickle(r"O:\OrlI\readmissions\preprocessed\ishpuzim_indicators\ishpuzim_preprocessed.pkl")
...
if IS_RUN_DIAG_COUNT:
    ...
    df_disch_count = pd.read_pickle (data_folder_path+ "diagnoses/diagnoses_for_model/diagnoses_count_disch_covid.pkl")
    df_adm_count=pd.read_pickle (data_folder_path+ "diagnoses/diagnoses_for_model/diagnoses_count_adm_covid.pkl")
    df_bg_count=pd.read_pickle (data_folder_path+ "diagnoses/diagnoses_for_model/diagnoses_count_bg_covid.pkl")
...
if IS_RUN_MEDS_COUNT:
    ...
    df_meds_disch_count = pd.read_pickle (data_folder_path+ "medication/meds_for_model/meds_count_disch_covid.pkl")
    df_meds_adm_count=pd.read_pickle (data_folder_path+ "medication/meds_for_model/meds_count_adm_covid.pkl")
    df_meds_hosp_count=pd.read_pickle (data_folder_path+ "medication/meds_for_model/meds_count_hosp_covid.pkl")
...
df_HS=pd.read_pickle(r"O:\OrlI\readmissions\preprocessed\HS\df_HS_pop_covid.pkl")
...
count_diag_cols = [col for col in df_merged.columns if 'COUNT_DIAG' in col]
if len(count_diag_cols)>0:    
    df_merged[count_diag_cols]=np.where(df_merged[count_diag_cols]==1,1,0)
        
count_meds_cols = [col for col in df_merged.columns if 'COUNT_MEDS' in col]
if len(count_meds_cols)>0:    
    df_merged[count_meds_cols]=np.where(df_merged[count_meds_cols]==1,1,0)
...
df_merged.drop(columns=['VS_CaseNum', 'LABS_CaseNum_x','LABS_CaseNum_y','LABS_CaseNum',
                        ...
                        "COUNT_MEDS_HOSP_Casenum"], inplace=True)#"PROCED_CaseNum","MEDS_HOSP_CaseNum","MEDS_ADM_CaseNum",,"SURGERY_CaseNum","DIAG_CaseNum_dis",
...
if IS_DUMMIES:
   ...
   # df_internal=df_merged[(df_merged["dept_cat_disch_internal"]==1) | (df_merged["dept_cat_disch_internal_ICU"]==1)]# |(df["dept_cat_disch_geriatrics"]==1)]
    ...
    df_all_covid=df_merged[(df_merged["dept_cat_disch_internal"]==1) |(df_merged["dept_cat_disch_internal_ICU"]==1) |
    ...
    df_all.to_pickle(output_path+"all_with_dummies.pkl")
    df_all_head.to_csv(output_path+"all_with_dummies_head.csv")
    df_internal.to_pickle(output_path+"internal_with_dummies.pkl")
    df_medical.to_pickle(output_path+"medical_with_dummies.pkl")
    df_surgical.to_pickle(output_path+"surgical_with_dummies.pkl")
    df_HS_internal.to_pickle(output_path+"HS_internal.pkl")

    df_merged.to_pickle((output_path+"all_with_dummies_covid_with_counts.pkl"))
    df_merged.to_csv((output_path+"all_with_dummies_covid_with_counts.csv"))
