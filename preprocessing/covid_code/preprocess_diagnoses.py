...
def f_preprocess_diagnoses():
    ...
    df_md=pd.read_excel(r'O:\OrlI\readmissions\code\support_files\Diagnosis_codes_MDClone_2021.xlsx')
    
    df_md=df_md[(df_md['Chapter'] != 'UNSPECIFIED')]
    df_md=df_md[(df_md['Chapter'] != 'Not specified')]
    ...
    df_disch_table=pivot_diagnoses(df_disch,'Chapter')
    ...
    df_adm_table=pivot_diagnoses(df_adm,'Chapter')
    ...
    df_disch_table.to_pickle(output_path+"disch_table_covid.pkl")
    df_adm_table.to_pickle(output_path+"adm_table_covid.pkl")
