...
IS_RUN_RESEARCH_POPULATION=True
...
global output_path
output_path="O:/OrlI/readmissions/preprocessed/population/population_3_months/"
...
def f_create_research_population():
    ...
    if os.path.isfile(s_output_path):
        df = pd.read_pickle(s_output_path)
        return df
    ...
    s_columns = 'CaseNum,\
                ...
                ReAdmPlan3MFlg,\
                ReAdm_Hosp_In3MFlg,\
                ReAdm_Hosp_In7D_CaseNum,\
                MedOrgTreeDisch,\
                OrgMedDischTatYahidaDesc,\
                Readm_Urgent_In3MFlg'
    ...
    s_cond = s_cond + "year(EnterDate) < 2021 OR (year(EnterDate) = 2021 AND month(EnterDate) <= 5)"
    ...
    # Removing non relevant departments
    ...
    df.loc[df['delta_death'] <= 60,'BASE_FLG'] = 0
    ...
    df.loc[(df.EnterDate.dt.year == 2021) & (df.EnterDate.dt.month == 5), 'BASE_FLG'] = 0
    ...
    df.loc[df['ReAdmPlan3MFlg'] == 1, 'BASE_FLG'] = 0
...
if IS_RUN_RESEARCH_POPULATION:
 ...
 #   df_base=pd.read_pickle(r"C:\Users\orlyk\readmissions\project\preprocessed\population\df_readmin_with_labels_base_slim.pkl")
    df_base=pd.read_pickle(r"C:\Users\orlyk\readmissions\project\preprocessed\population\df_readmin_with_labels_base.pkl")
