...
global output_path
output_path="O:/OrlI/readmissions/preprocessed/ventilation/mechanical_chameleon/"
...
def f_get_ventilation_chameleon_full():
    ...
    if file.is_file():
      ...
      where Parameter in (5309,5311,327)
         """
    ...
    where Parameter_Name='TV_PATIENT' OR Parameter_Name='TV_PATIENT'"""
      ...
      where Parameter in (5309,5311,327,13)"""
  ...
  FROM [DWH_PRD].[dbo].[Chameleon_Fact_MedicalRecords]"""
        ...
        names=["monitor","monitor_arch","monitor_device","monitor_device_arch","v_exec"]
        df= pd.DataFrame()
        ...
        df=df.loc[(df['Parameter'] == 5309)| (df['Parameter'] == 5311) | (df['Parameter'] == 327)]
        ...
        #df.to_pickle(dwr_path + "chameleon_data_mech_ventilation.pkl")
        return df

def get_mech_vent_chameleon_pop(l_casenum=[]):
    df = f_get_ventilation_chameleon_full()
    if len(l_casenum) > 0:
        df = df[df['CaseNum'].isin(l_casenum)]
        df.to_pickle(output_path+"mech_vent_pop.pkl")
        df.to_csv(output_path+"mech_vent_pop.csv")
